...
if version_info >= (2,6,0):
    def swig_import_helper():
        # NOTE: Changed _ok to FAKE_ok, so the REAL _ok Opal Kelly library file IS NOT
        # ACTUALLY RUNNING. FAKE_ok is an emulator!
        from os.path import dirname
        import imp
        fp = None
        try:                                        #   vvvvvvvvv This should be just "_ok" !
            fp, pathname, description = imp.find_module('FAKE_ok', [dirname(__file__)])
        except ImportError:
            import _ok
            return _ok
        if fp is not None:
            try:
                _mod = imp.load_module('FAKE_ok', fp, pathname, description)
            finally:
                fp.close()
            return _mod
    ...
    import FAKE_ok # <<<Should be just _ok
del version_info
try:
